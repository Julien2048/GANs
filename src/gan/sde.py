...
class SimpleSDE(SDE):
    ...
    def marginal_prob(self, x, t):
        mean = x
        std = torch.sqrt(
            (self.sigma ** (2 * torch.tensor(t, device=self.device)) - 1.0)
            / 2.0
            / np.log(self.sigma)
        )
        return mean, std
    ...
    def prior_sampling(self, shape):
        ...
        return (
            torch.randn(shape[0], 1, 28, 28, device=self.device)
...
class VESDE(SDE):
    def __init__(self, sigma_min=0.01, sigma_max=50, N=1000, device: str = "cuda"):
